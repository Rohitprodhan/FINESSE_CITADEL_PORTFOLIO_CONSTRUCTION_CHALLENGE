...
# -------------------------------------------------------------
DATA_DIR = "/Users/mdsohelmondal/IITB_Portfolio_Challenge/data"
...
# -------------------------------------------------------------
# 1. Load Local Cached Data (No External Downloads)
# -------------------------------------------------------------
...
# Align backtest strictly to the competition window: 2021 to 2025
backtest_dates = adj_close.loc["2021-01-01":"2025-12-31"].index
all_dates = adj_close.index

# Schedule monthly rebalancing (first trading day of each month)
rebalance_dates_raw = (
    adj_close.loc["2021-01-01":"2025-12-31"].resample("MS").first().index
)
rebalance_dates = [
...
print("Running 5-Year Systematic Equity Backtest (2021-2025)...")
...
cagr = ((final_val / INITIAL_CAPITAL) ** (1.0 / 5.0)) - 1.0
...
# Print Summary Table
print("\n" + "=" * 55)
print("          OFFICIAL BACKTEST SUMMARY (2021-2025)")
print("=" * 55)
print(df_summary.to_string(index=False))
print("=" * 55)

# -------------------------------------------------------------
# 4. Save Outputs & Equity Curve Plot
# -------------------------------------------------------------
df_summary.to_csv(os.path.join(DATA_DIR, "performance_summary.csv"), index=False)
df_results.to_csv(os.path.join(DATA_DIR, "backtest_curve.csv"))
df_trades.to_csv(os.path.join(DATA_DIR, "trades_log.csv"), index=False)
pd.DataFrame(monthly_holdings_log).to_csv(
    os.path.join(DATA_DIR, "monthly_holdings.csv"), index=False
)
...
plt.plot(
    ...
    label="Strategy Portfolio (₹ Cr)",
    color="#0055aa",
    lw=2,
)
plt.title("Portfolio Growth from 2021 to 2025", fontsize=14, fontweight="bold")
plt.xlabel("Date")
...
print("1. performance_summary.csv  (Executive headline metrics)")
print("2. backtest_curve.csv        (Daily portfolio valuation & returns)")
print("3. trades_log.csv            (Order execution log with fees)")
print("4. monthly_holdings.csv      (Month-by-month target allocations)")
print("5. equity_curve.png          (High-resolution compounding plot)")